...
```text
N=1000: Time to link 1000 children: 76.3017 ms
N=10000: Time to link 10000 children: 7457.5643 ms
N=100000: Time to link 100000 children: 797695.1953 ms
```
...
def measure_index_performance(num_elements):
    ...
    for element in elements:
        idx.find(element.name) # Assuming find by name
...
def build_dense_graph(num_nodes):
    nodes = [s.get(f"node_{i}") for i in range(num_nodes)]
...
def build_linear_graph(num_nodes, prefix):
    nodes = [s.get(f"{prefix}_node_{i}") for i in range(num_nodes)]

...
from sym.builtins.path import SymbolPathMixin
...
def disconnected_graph():
    # These syms are not connected to each other, nor to simple_graph
...
def test_match_dfs(simple_graph):
    ...
    matched_syms = list(a.match(predicate, traversal='dfs'))
    # DFS order: A, B, C, D. Matches: B, C
    assert matched_syms == [b, c]
...
def test_match_bfs(simple_graph):
    ...
    matched_syms = list(a.match(predicate, traversal='bfs'))
    # BFS order: A, B, D, C. Matches: B, D
    assert matched_syms == [b, d]

def test_match_no_match(simple_graph):
    a, _, _, _ = simple_graph
    predicate = lambda sym: sym.name == "Z"
    matched_syms = list(a.match(predicate))
    assert matched_syms == []
...
def test_match_all_match(simple_graph):
    ...
    matched_syms = list(a.match(predicate, traversal='dfs'))
    assert set(matched_syms) == {a, b, c, d}

import pytest
from sym.core.sym import Symbol, GraphTraversal, s, to_sym
from sym.core.base_sym import Symbol as BaseSymbol
from sym.core.lazy import SENTINEL
from sym.builtins.avl_tree import AVLTree
from weakref import WeakValueDictionary
import threading
import datetime

# Re-use the setup_and_teardown fixture from test_base_sym.py
# This ensures a clean state for Symbol class-level attributes before each test
@pytest.fixture(autouse=True)
def setup_and_teardown_sym_class():
    ...
    # Clear any remaining syms from the pool and numbered tree to ensure complete isolation
...
def test_sym_metadata_property():
...
def test_sym_elevated_attributes_set_and_get():
...
def test_sym_elevated_attributes_overwrite_existing():
...
def test_sym_elevated_attributes_delete():
...
def test_sym_elevated_attributes_delete_non_existent():
    sym = Symbol("test_elevated_attrs_delete_non_existent")
    with pytest.raises(AttributeError):
        del sym.non_existent_attr

def test_sym_append_child():
...
def test_sym_append_lazy_sym_child():
...
def test_sym_append_non_sym_object():
    parent = Symbol("parent_non_sym")
    non_sym_obj = "string_child"
    child_sym = parent.append(non_sym_obj)
...
def test_sym_append_duplicate_child():
...
def test_sym_add_duplicate_child():
...
def test_sym_relate_with_specific_how():
    s1 = Symbol("s1_how")
    s2 = Symbol("s2_how")
...
def test_sym_unrelate_with_specific_how():

...
from sym.core.base_sym import Symbol, _to_sym, AVLTree

@pytest.fixture(autouse=True)
def setup_and_teardown_base_sym():
...
def test_sym_pool_and_numbered_insertion():
...
def test_sym_less_than_comparison():
    s1 = Symbol("lt_test_1")
    s2 = Symbol("lt_test_2")
    s3 = Symbol("lt_test_3")
...
def test_to_sym_existing_sym():
    s_orig = Symbol("existing")
    s_converted = _to_sym(s_orig)
    assert s_converted is s_orig

def test_to_sym_from_string():
    s_converted = _to_sym("from_string")
    assert isinstance(s_converted, Symbol)
    assert s_converted.name == "from_string"

def test_to_sym_from_object_with_name_attribute():
    class HasName:
        def __init__(self, name):
            self.name = name
    obj = HasName("object_with_name")
    s_converted = _to_sym(obj)
    assert isinstance(s_converted, Symbol)
    assert s_converted.name == "object_with_name"

def test_to_sym_type_error_for_unconvertible_type():

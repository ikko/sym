Feature: Symbol Happy Path Functionality
  ...
  So that I can build reliable symbolic systems

  Scenario: Symbol creation and interning
    Given a symbol name "test_symbol_1"
    When a Symbol is created with "test_symbol_1"
    Then an instance of Symbol is returned
    And the symbol's name is "test_symbol_1"
    When another Symbol is created with the same name "test_symbol_1"
    Then the same Symbol instance is returned (interning)
    When a Symbol is created with a different name "test_symbol_2"
    Then a new Symbol instance is returned
    And the new symbol's name is "test_symbol_2"

  Scenario: Symbol equality and hashing
    Given two Symbols with the same name "equal_symbol"
    And one Symbol with a different name "another_symbol"
  ...
  Scenario: Symbol string representation
    Given a Symbol named "repr_symbol"
    When the Symbol is converted to a string
    Then the string representation should be "repr_symbol"
    When the Symbol's repr is called
    Then the repr representation should be ":repr_symbol"
  ...
  Scenario: SymbolNamespace functionality
    ...
    When a Symbol is accessed via attribute (e.g., "my_attribute_symbol")
    Then an instance of Symbol is returned
    And the symbol's name matches the attribute name
    When a Symbol is accessed via item (e.g., "my_item_symbol")
    Then an instance of Symbol is returned
    And the symbol's name matches the item name

Feature: Symbol Happy Path Functionality
  ...
  So that I can build reliable symbic systems

  Scenario: Symbol creation and interning
    Given a symb name "test_symb_1"
    When a Symbol is created with "test_symb_1"
    Then an instance of Symbol is returned
    And the symb's name is "test_symb_1"
    When another Symbol is created with the same name "test_symb_1"
    Then the same Symbol instance is returned (interning)
    When a Symbol is created with a different name "test_symb_2"
    Then a new Symbol instance is returned
    And the new symb's name is "test_symb_2"

  Scenario: Symbol equality and hashing
    Given two Symbols with the same name "equal_symb"
    And one Symbol with a different name "another_symb"
  ...
  Scenario: Symbol string representation
    Given a Symbol named "repr_symb"
    When the Symbol is converted to a string
    Then the string representation should be "repr_symb"
    When the Symbol's repr is called
    Then the repr representation should be ":repr_symb"
  ...
  Scenario: SymbolNamespace functionality
    ...
    When a Symbol is accessed via attribute (e.g., "my_attribute_symb")
    Then an instance of Symbol is returned
    And the symb's name matches the attribute name
    When a Symbol is accessed via item (e.g., "my_item_symb")
    Then an instance of Symbol is returned
    And the symb's name matches the item name

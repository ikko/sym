...
def setup_and_teardown(request):
    ...
    # If the test function has a specific set of syms to use, add them
    if hasattr(request, 'param'):
        for sym in request.param:
            # Assuming sym has a ._position attribute for its weight
            Symbol(sym.name, sym.origin) # Re-create sym to ensure it's added to the new AVLTree and pool
